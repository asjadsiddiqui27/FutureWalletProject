...
const VerifySecretPhrase = props => {
  //   console.log('secondddddddddddd', props.route.params.wordd);
  const {wordd} = props.route.params;
  const abc = [...wordd];
  const [dataArray, setMainArray] = useState([]);
  const [newArray, setNewArray] = useState([]);

  console.log(newArray);
  //   useEffect(() => {
  //     if (JSON.stringify(wordd) == JSON.stringify(newArray)) {
  //       Alert.alert('hello');
  //     }
  //   }, [newArray]);
  ...
  // btn end //
  const handleBtn = () => {
    const worddData = wordd.map((item, index) => {
      return item;
    });

    const machingData = newArray.map((item, index) => {
      return item.Item;
    });

    if (JSON.stringify(worddData) === JSON.stringify(machingData)) {
      Alert.alert('Success');
  ...
  useEffect(() => {
    ...
    const shuffledArray = shuffleArray(abc);
    setMainArray(shuffledArray);
        ...
        <View style={styles.main_body_container}>
          <View style={styles.text_heading_container}>
            <Text
              ...
              style={styles.text_main_heading}>
              {Strings.English.verifyPhrase.taptheWord}
            </Text>
          </View>
          ...
          <View style={styles.body_main_container}>
            {newArray.map((item, index) => (
              <Button
                ...
                buttonStyle={styles.btn_style_upper}
                name_2={item.index + 1 + '.'}
                name={item.Item}
                onPress={() => {
                  console.log(item), resetItem(item);
                }}
              />
            ))}
          </View>
            ...
            {dataArray.map((item, index) =>
              item != '' ? (
                <Button
                  ...
                  name={item}
                  onPress={() => {
                    console.log(item), setItem(item, index);
                  }}
                />
              ) : (
